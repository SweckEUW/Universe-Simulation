...
//		this.multiply(all);
	...
	public void changeToViewMatrix(Camera camera) {
		...
		//negation of transformation to move the scene correctly 
		if (camera.isExploreModeEnabled()) {
	
			//rotate as frist person
			...
			this.rotateArroundPoint(camera.getExploreMode().getPlanetCenter(), new Vector3f(0,camera.getExploreMode().getCurentPositionY(),0));
			this.rotateArroundPoint(camera.getExploreMode().getPlanetCenter(), new Vector3f(camera.getExploreMode().getCurentPositionX(),0,0));   		
							
		}else if (camera.getLookAtPlanetMode()) {
			this.translate(-camera.getX(), -camera.getY(), -camera.getZ()); 		
				
			this.rotateArroundPoint(LookAtPlanet.getCenter(), new Vector3f(camera.getRotateX(), camera.getRotateY(), camera.getRotateZ()));
		}else if (camera.getCameraControls().isCameraModeOrigin()) {	
			this.translate(-camera.getX(), -camera.getY(), -camera.getZ()); 
