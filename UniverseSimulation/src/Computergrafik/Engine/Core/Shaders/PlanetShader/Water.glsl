...
flat	 out vec3 modelSpacePosition;
flat out vec3 unitModelSpaceNormal;	
flat out vec3 flatColor;	
flat	 out vec3 cameraDirection;
												 
void main(void){
			
	gl_Position = projectionMatrix * modelViewProjectionMatrix *(vec4(position.x*height,position.y*height,position.z*height,1));
			
	modelSpacePosition= (modelMatrix * vec4(position,1)).xyz;
	
	flatColor=color;
	
	unitModelSpaceNormal = normalize((inverse(transpose(modelMatrix)) * vec4(normal,0)).xyz); 		
...
flat in vec3 unitModelSpaceNormal;	
flat in vec3 flatColor;
	flat in vec3 modelSpacePosition;	
flat	 in vec3 cameraDirection;
...
void main(void){	
	...
	for(int i = 0; i < numberOfDirectionalLights ; i++)
		fragmentColor += calculateDirectionalLight(directionalLights[i],material,unitModelSpaceNormal,cameraDirection,modelSpacePosition);	
	for(int i = 0; i < numberOfPointLights ; i++)
		fragmentColor += calculatePointLight(pointLights[i],material,unitModelSpaceNormal,cameraDirection,modelSpacePosition);
	
	fragmentColor+=material.emissionColor;
	fragmentColor+=material.ambientColor*ambientLightColor;
	
	finalColor=vec4(flatColor*fragmentColor,0.5);					
};
