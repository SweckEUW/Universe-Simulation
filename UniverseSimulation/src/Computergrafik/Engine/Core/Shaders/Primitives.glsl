...
void main(void){
	...
	modelSpaceNormal = normalize((inverse(transpose(modelMatrix)) * vec4(normal,0)).xyz); 		
	//modelSpaceNormal = normalize((modelMatrix * vec4(normal,0)).xyz);	
...
void main(void){	
	...
	vec3 unitModelSpaceNormal = normalize(modelSpaceNormal);
	...
	finalColor=vec4(objectColor*fragmentColor,1);										
};