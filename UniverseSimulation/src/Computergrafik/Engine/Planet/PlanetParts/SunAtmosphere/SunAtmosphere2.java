...
public class SunAtmosphere2 extends SunAtmosphere {

	private static final String FILE = "PlanetParts/sunAtmosphere2";
	private static final float SIZE = 1.2f;
	private static final int NUMBER_OF_MODELS = 1;
	
	public SunAtmosphere2(Planet planet) {
	...
	public void calculateColors() {
		...
		for (int i = 0; i < colors[0].length; i+=3) {
			...
			colors[0][i+1]=0f/255f*(0.9f+((float)Math.random()*(1f-0.9f)));
			colors[0][i+2]=0f/255f*(0.9f+((float)Math.random()*(1f-0.9f)));
		}
