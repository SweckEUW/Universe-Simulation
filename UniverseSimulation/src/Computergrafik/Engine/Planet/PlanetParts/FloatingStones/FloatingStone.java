...
public abstract class FloatingStone extends PlanetPart{
	...
	public FloatingStone(Planet planet,int numberOfModels) {
		super(numberOfModels,planet,new BasicShader(FILE));
		speedX=(float)Math.random() *0.7f; 
		speedY=(float)Math.random() *0.7f; 
		speedZ=(float)Math.random() *0.7f; 
	...
	private void addNoise() {
		...
		for (int i = 0; i < newVertices.length; i+=3) {
			float randomNoise = 0.97f+((float)Math.random()*(1.02f-0.97f));
