...
public class FloatingStone1 extends FloatingStone {

	private static final String FILE = "PlanetParts/FloatingStone1";
	private static final float SIZE = 0.3f;
	private static final int NUMBER_OF_MODELS = 1;
	
	public FloatingStone1(Planet planet) {
	...
	public void calculateColors() {
		...
		for (int i = 0; i < colors[0].length; i+=3) {
			colors[0][i]=166f/255f*(0.7f+((float)Math.random()*(1f-0.7f)));
			colors[0][i+1]=226f/255f*(0.7f+((float)Math.random()*(1f-0.7f)));
			colors[0][i+2]=226f/255f*(0.7f+((float)Math.random()*(1f-0.7f)));
		}
