...
public class FloatingStone2 extends FloatingStone {

	private static final String FILE = "PlanetParts/FloatingStone2";
	private static final float SIZE = 0.3f;
	private static final int NUMBER_OF_MODELS = 1;
	
	public FloatingStone2(Planet planet) {
	...
	public void calculateColors() {
		...
		for (int i = 0; i < colors[0].length; i+=3) {
			colors[0][i]=186f/255f*(0.7f+((float)Math.random()*(1f-0.7f)));
			colors[0][i+1]=186f/255f*(0.7f+((float)Math.random()*(1f-0.7f)));
			colors[0][i+2]=136f/255f*(0.7f+((float)Math.random()*(1f-0.7f)));
		}
	...
	protected void setModels() {
		Material material = new Material(new Vector3f(0.2f), new Vector3f(0.7f), new Vector3f(1,1,0), 10);
