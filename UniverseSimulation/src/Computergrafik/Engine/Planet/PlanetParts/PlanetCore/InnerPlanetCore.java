...
public class InnerPlanetCore extends PlanetPart{

	private static final String SHADER_FILE = "PlanetShader/Planet";
	private static final String FILE = "PlanetParts/InnerCore";
	...
	public InnerPlanetCore(Planet planet) {
	...
	public void calculateColors() {
		...
		for (int i = 0; i < colors[0].length; i+=3) {
			colors[0][i]=133f/255f*(0.5f+((float)Math.random()*(1f-0.5f)));
			colors[0][i+1]=24f/255f*(0.5f+((float)Math.random()*(1f-0.5f)));
			colors[0][i+2]=52f/255f*(0.5f+((float)Math.random()*(1f-0.5f)));
		}
