...
public class Stone1 extends PlanetEntity{

	private static final String FILE = "PlanetEntities/Stones/Stone1";
	private static final int NUMBER_OF_MODELS = 1;
	
	public Stone1(Planet planet,int instances) {
	...
	public void calculateColors() {	
		...
		for (int i = 0; i < colors.length; i+=3) { 
			colors[i]=148f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+1]=148f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+2]=145f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
		}
