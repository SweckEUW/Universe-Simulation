package Computergrafik.Engine.Planet.PlanetEntities.Stones;
...
public class Stone3 extends PlanetEntity{

	private static final String FILE = "PlanetEntities/Stones/Stone3";
	private static final int NUMBER_OF_MODELS = 1;
	
	public Stone3(Planet planet,int instances) {
	...
	public void calculateColors() {	
		...
		for (int i = 0; i < colors.length; i+=3) { 
			...
			colors[i+1]=161f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+2]=157f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
		}
	...
	protected void setModels() {
		...
		Material materialStamm = new Material(new Vector3f(0.2f), new Vector3f(1), new Vector3f(0.1f,0.1f,0.1f), 200);
