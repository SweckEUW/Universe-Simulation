...
public class Mushroom1 extends PlanetEntity{

	private static final String FILE = "PlanetEntities/Mushrooms/Mushroom1";
	private static final int NUMBER_OF_MODELS = 2;
	
	public Mushroom1(Planet planet,int instances) {
	...
	public void calculateColors() {	
		...
		colorOffset=0.9f;
		for (int i = 0; i < colors.length; i+=3) {
			colors[i]=208f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+1]=183f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+2]=104f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
		}
