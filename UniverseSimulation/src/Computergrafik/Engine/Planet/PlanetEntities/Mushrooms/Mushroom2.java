...
public class Mushroom2 extends PlanetEntity{

	private static final String FILE = "PlanetEntities/Mushrooms/Mushroom2";
	private static final int NUMBER_OF_MODELS = 2;
	
	public Mushroom2(Planet planet,int instances) {
	...
	public void calculateColors() {	
		...
		for (int i = 0; i < colors.length; i+=3) {
			colors[i]=255f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+1]=228f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+2]=196f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
		}
