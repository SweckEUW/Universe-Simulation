...
public class Mushroom5 extends PlanetEntity{

	private static final String FILE = "PlanetEntities/Mushrooms/Mushroom5";
	private static final int NUMBER_OF_MODELS = 2;
	
	public Mushroom5(Planet planet,int instances) {
	...
	public void calculateColors() {	
		...
		for (int i = 0; i < colors.length; i+=3) {
			colors[i]=182f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+1]=64f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+2]=64f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
		}
		...
		colorOffset=0.9f;
		for (int i = 0; i < colors.length; i+=3) {
			colors[i]=238f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+1]=225f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+2]=190f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
		}
