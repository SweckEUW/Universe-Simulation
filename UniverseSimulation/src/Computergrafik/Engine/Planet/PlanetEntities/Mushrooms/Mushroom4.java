...
public class Mushroom4 extends PlanetEntity{

	private static final String FILE = "PlanetEntities/Mushrooms/Mushroom4";
	private static final int NUMBER_OF_MODELS = 2;
	
	public Mushroom4(Planet planet,int instances) {
	...
	public void calculateColors() {	
		...
		for (int i = 0; i < colors.length; i+=3) {
			colors[i]=190f/255f*(colorOffset+((float)Math.random()*(1f-colorOffset)));
		...
		for (int i = 0; i < colors.length; i+=3) {
			...
			colors[i+1]=1*(colorOffset+((float)Math.random()*(1f-colorOffset)));
			colors[i+2]=1*(colorOffset+((float)Math.random()*(1f-colorOffset)));
		}
