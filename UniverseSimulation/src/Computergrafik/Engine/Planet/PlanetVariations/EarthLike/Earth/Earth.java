package Computergrafik.Engine.Planet.PlanetVariations.EarthLike.Earth;
...
import Computergrafik.Engine.Planet.PlanetParts.Water.SphericalWater;

public class Earth extends Planet {

	private int NUMBER_OF_TREES = 1500;
	private int NUMBER_OF_FLOWERS = 800;
	private int NUMBER_OF_MUSHROOMS = 800;
	private int NUMBER_OF_GRASS = 1000; 
	
	public Earth(Planet planet,Vector3f translation,float scale, Material material,Vector3f orbitAxis) {
		super(new EarthMesh(scale), planet,translation,scale, material,orbitAxis);
	...
	public Earth(Vector3f translation,float scale, Material material) {
		super(new EarthMesh(scale),translation,scale, material);
		NUMBER_OF_TREES=(int)(Math.random()*NUMBER_OF_TREES);
		NUMBER_OF_FLOWERS=(int)(Math.random()*NUMBER_OF_FLOWERS);
		NUMBER_OF_MUSHROOMS=(int)(Math.random()*NUMBER_OF_MUSHROOMS);
		NUMBER_OF_GRASS=(int)(Math.random()*NUMBER_OF_GRASS);
...
//		this.addEntity(new Flower2(this, flower2));
	...
	private void addParts() {
		this.addPart(new SphericalWater(this));
	}
